...
class EntityPrinter
{
    ...
    public function makeEntityIdentifierFromClassAndId(string $fqcn, mixed $id): string
    {
        ...
        if ($e) {
            ...
            if(is_object($idValue) && ! ($idValue instanceof \Stringable)){
                return $ref->getShortName() . " ($identification->" . get_class($idValue) . ')';
            } else if (is_object($idValue) && ($idValue instanceof \Stringable || method_exists($idValue, '__toString'))) {
                return $ref->getShortName() . " ($identification->" . $idValue->__toString() . ')';
            } else if (!is_object($idValue)) {
                if ($e instanceof \Stringable || method_exists($e, '__toString')) {
                    return $ref->getShortName() . '#' . $id . ' (' . $e->__toString() . ')';
                } else if (method_exists($e, 'getName')) {
                    return $ref->getShortName() . '#' . $id . ' (' . $e->getName() . ')';
                } else if (method_exists($e, 'getTitle')) {
                    return $ref->getShortName() . '#' . $id . ' (' . $e->getTitle() . ')';
                } 
